...
class VLMAgent:
    def __init__(
        ...
        self.is_ollama = (model == "omniparser + ollama")
    ...
    def _run_model(self, messages: list, system: str, use_json_bridge: bool = True, stage: str = "action"):
        if self.is_ollama:
            ...
            # Optional second-stage JSON planning model.
            if (
                use_json_bridge
                and
    ...
    def _normalize_action_json(self, vlm_response_json: dict, parsed_screen: dict):
        ...
        if "Box ID" in normalized:
            ...
            if resolved is None:
                # Force safe fallback instead of blind click at current cursor.
        ...
        # Desktop icon safety: prefer double_click for icon-like targets.
    ...
    def _completion_check_override(
        ...
        self.output_callback(
            "<i>Completion check: task not complete, continuing with another action.</i>",
            sender="bot",
        )
        return override
